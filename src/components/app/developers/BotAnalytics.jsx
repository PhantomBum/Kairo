    ...
    <div className="space-y-6">
      ...
      {bot.commands?.length > 0 && (
        <div className="rounded-2xl p-5" style={{ background: 'var(--bg-glass)', border: '1px solid var(--border)' }}>
          <h4 className="text-[11px] font-semibold uppercase tracking-[0.08em] mb-3" style={{ color: 'var(--text-muted)', fontFamily: 'monospace' }}>Commands</h4>
          <div className="space-y-2">
            {bot.commands.map(cmd => (
              <div key={cmd.id} className="flex items-center justify-between py-1.5">
                ...
                <span className="text-[11px] px-2 py-0.5 rounded-full" style={{ background: cmd.enabled ? 'rgba(123,201,164,0.1)' : 'rgba(201,123,123,0.1)', color: cmd.enabled ? 'var(--accent-green)' : 'var(--accent-red)' }}>
      ...
      <div className="rounded-2xl p-5" style={{ background: 'var(--bg-glass)', border: '1px solid var(--border)' }}>
        <h4 className="text-[11px] font-semibold uppercase tracking-[0.08em] mb-3" style={{ color: 'var(--text-muted)', fontFamily: 'monospace' }}>Installations ({installations.length})</h4>
          ...
          <div className="space-y-2">
            {installations.map(inst => (
              <div key={inst.id} className="flex items-center justify-between py-1.5" style={{ borderBottom: '1px solid var(--border)' }}>
                ...
                <span className="text-[11px]" style={{ color: 'var(--text-faint)' }}>{new Date(inst.installed_at).toLocaleDateString()}</span>
              </div>
            ))}
          </div>
        )}
      ...
      {bot.reviews?.length > 0 && (
        <div className="rounded-2xl p-5" style={{ background: 'var(--bg-glass)', border: '1px solid var(--border)' }}>
          <h4 className="text-[11px] font-semibold uppercase tracking-[0.08em] mb-3" style={{ color: 'var(--text-muted)', fontFamily: 'monospace' }}>Reviews</h4>
            ...
            {bot.reviews.map((r, i) => (
              <div key={i} className="p-3 rounded-xl" style={{ background: 'var(--bg-glass-strong)' }}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[11px]" style={{ color: 'var(--accent-amber)' }}>{'★'.repeat(r.rating)}{'☆'.repeat(5 - r.rating)}</span>
                </div>
