...
const MessageBubble = memo(function MessageBubble({ message, compact, isOwn, onReply, onEdit, onDelete, onReact, onPin, onStar, currentUserId, onProfileClick, isEditing, onEditSave, onEditCancel, onImageClick, onLinkClick, onHighlight }) {
  ...
  const roleColor = message.author_badges?.includes('owner') ? colors.warning
    : message.author_badges?.includes('admin') ? colors.info : colors.text.primary;
            ...
            {isDeleted ? (
              ...
              <div>
                <textarea value={editText} onChange={e => setEditText(e.target.value)}
                  ...
                  className="w-full text-[14px] rounded px-3 py-2 outline-none resize-none" style={{ background: colors.bg.overlay, color: colors.text.primary, border: `1px solid ${colors.accent.primary}` }}
                  rows={2} autoFocus />
          ...
          {hovered && !isEditing && !isDeleted && (
            <div className="absolute -top-3 right-4 flex items-center p-px rounded-lg gap-px z-10 k-fade-in"
              ...
              {isOwn && <>
                ...
                <button onClick={() => onDelete(message)} className="w-7 h-7 flex items-center justify-center rounded-sm hover:bg-[rgba(255,255,255,0.06)]" title="Delete"><Trash2 className="w-3.5 h-3.5" style={{ color: colors.danger }} /></button>
              </>}
            </div>
          )}
        </div>
      ...
      <ContextMenuContent className="w-52 p-1 rounded-lg" style={{ background: colors.bg.float, border: `1px solid ${colors.border.strong}`, boxShadow: '0 8px 32px rgba(0,0,0,0.6)' }}>
        {!isDeleted && <>
          <div className="flex items-center gap-0.5 px-1 py-1 mb-1">
            {['👍', '❤️', '😂', '🔥', '👀', '✨'].map(e => (
              <button key={e} onClick={() => onReact(message, e)} className="w-7 h-7 flex items-center justify-center rounded-sm text-base hover:bg-[rgba(255,255,255,0.1)]">{e}</button>
            ))}
          </div>
          <ContextMenuSeparator style={{ background: 'rgba(255,255,255,0.06)', margin: '4px 0' }} />
          <ContextMenuItem onClick={() => onReply(message)} className="text-[13px] gap-2 rounded-sm px-2 py-1.5" style={{ color: colors.text.secondary }}><Reply className="w-4 h-4 opacity-50" /> Reply</ContextMenuItem>
          <ContextMenuItem onClick={() => navigator.clipboard.writeText(message.content || '')} className="text-[13px] gap-2 rounded-sm px-2 py-1.5" style={{ color: colors.text.secondary }}><Copy className="w-4 h-4 opacity-50" /> Copy Text</ContextMenuItem>
        </>}
        ...
        {isOwn && !isDeleted && <>
          <ContextMenuSeparator style={{ background: 'rgba(255,255,255,0.06)', margin: '4px 0' }} />
          <ContextMenuItem onClick={() => onEdit(message)} className="text-[13px] gap-2 rounded-sm px-2 py-1.5" style={{ color: colors.text.secondary }}><Pencil className="w-4 h-4 opacity-50" /> Edit</ContextMenuItem>
          <ContextMenuItem onClick={() => onDelete(message)} className="text-[13px] gap-2 rounded-sm px-2 py-1.5" style={{ color: colors.danger }}><Trash2 className="w-4 h-4 opacity-50" /> Delete</ContextMenuItem>
        </>}
      </ContextMenuContent>
    </ContextMenu>
  );
