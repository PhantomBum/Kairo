...
import { Hash, Volume2, ChevronDown, ChevronRight, Megaphone, MessagesSquare, Settings, Plus, Shield, UserPlus, Bell, BellOff, Sparkles } from 'lucide-react';
    ...
    <div className="mb-0.5">
      <div className="flex items-center group px-0.5">
        <button onClick={() => setOpen(!open)}
          className="flex-1 flex items-center gap-0.5 px-1.5 py-[6px] text-[11px] font-semibold uppercase tracking-[0.05em] text-zinc-500 hover:text-zinc-300 transition-colors">
          ...
          {channels.map(ch => {
            ...
            return (
              <button key={ch.id} onClick={() => onChannelClick(ch)}
                ...
                {ch.type === 'voice' && (
                  <span className="text-[9px] text-zinc-600 opacity-0 group-hover/ch:opacity-100 transition-opacity">0 users</span>
                )}
              </button>
            );
          })}
