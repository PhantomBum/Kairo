...
export default function CreateGroupDMModal({ onClose, friends = [], onCreate }) {
  ...
  const handleCreate = () => {
    ...
    const participants = selected.map(f => ({
      user_id: f.friend_id, user_email: f.friend_email, user_name: f.friend_name, avatar: f.friend_avatar,
    }));
      ...
      <div className="relative w-full max-w-sm mx-4 rounded-xl overflow-hidden" style={{ background: '#1a1a1a' }}>
        <div className="p-5">
          ...
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Group name (optional)"
            className="w-full px-3 py-2 rounded-lg text-sm text-white placeholder:text-zinc-600 focus:outline-none mb-3" style={{ background: '#111' }} />
          ...
          <div className="max-h-60 overflow-y-auto space-y-1">
            {friends.map(f => {
              ...
              return (
                <button key={f.id} onClick={() => toggle(f)} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors"
                  style={{ background: isSel ? 'rgba(255,255,255,0.08)' : 'transparent' }}>
            ...
            {friends.length === 0 && <div className="text-center py-6 text-sm text-zinc-500">No friends yet</div>}
          </div>
        </div>
