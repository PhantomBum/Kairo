        ...
        <div className="flex gap-3">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center flex-shrink-0">
            {parentMessage.author_avatar ? (
              ...
              <span className="text-white text-sm font-medium">{parentMessage.author_name?.charAt(0)}</span>
            )}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="font-semibold text-white">{parentMessage.author_name}</span>
              <span className="text-xs text-zinc-500">{format(new Date(parentMessage.created_date), 'MMM d, h:mm a')}</span>
            </div>
            <p className="text-sm text-zinc-300">{parentMessage.content}</p>
            <p className="text-xs text-indigo-400 mt-2">{thread.message_count || 0} {thread.message_count === 1 ? 'reply' : 'replies'}</p>
          </div>
        </div>
      </div>

      <div className="flex-1 flex flex-col min-h-0">
        <MessageList messages={threadMessages} currentUserId={currentUser.id} onReply={setReplyTo} onReact={() => {}} />
        <MessageInput channelName="Thread" replyTo={replyTo} onCancelReply={() => setReplyTo(null)} onSendMessage={(data) => sendMessageMutation.mutate(data)} placeholder="Reply to thread..." />
      </div>
    </motion.div>
  );
}