...
export default function PinnedMessagesPanel({ 
  ...
  return (
    <AnimatePresence>
      <div className="absolute top-0 right-0 h-full w-80 bg-[#0f0f11] border-l border-zinc-800 flex flex-col z-40">
        <motion.div
          initial={{ x: '100%' }}
          animate={{ x: 0 }}
          exit={{ x: '100%' }}
            ...
            {messages.length === 0 ? (
              ...
              messages.map((msg) => (
                <motion.div
                  ...
                  className="p-3 bg-zinc-800/30 rounded-lg hover:bg-zinc-800/50 transition-colors group cursor-pointer"
                ...
                >
                  <div className="flex items-start gap-2 mb-2">
                    <div className="w-6 h-6 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center flex-shrink-0">
                      {msg.author_avatar ? (
                        <img src={msg.author_avatar} alt="" className="w-full h-full rounded-full" />
                      ) : (
