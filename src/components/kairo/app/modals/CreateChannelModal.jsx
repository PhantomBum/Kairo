...
export default function CreateChannelModal({ isOpen, onClose, onCreate, categories = [], defaultCategoryId }) {
      ...
      <div className="relative w-full max-w-md mx-4 rounded-xl overflow-hidden" style={{ background: '#1a1a1a' }}>
          ...
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {types.map(t => (
                  <button key={t.id} onClick={() => setType(t.id)}
                    ...
                    style={{
                      background: type === t.id ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.03)',
                      border: `1px solid ${type === t.id ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.06)'}`,
                    }}>
            ...
            <div>
              ...
              <input value={name} onChange={e => setName(e.target.value.toLowerCase().replace(/\s+/g, '-'))}
                ...
                className="w-full px-3 py-2 rounded-lg text-sm text-white placeholder:text-zinc-600 focus:outline-none"
        ...
        <div className="flex justify-end gap-2 px-5 py-4" style={{ background: '#151515' }}>
          ...
          <button onClick={handleCreate} disabled={!name.trim() || creating}
            className="px-4 py-2 rounded-lg text-sm font-medium text-black disabled:opacity-50 transition-colors"
