...
export default function AddFriendModal({ isOpen, onClose, onSendRequest, currentUserId }) {
  ...
  const handleSendRequest = async (user) => {
    ...
    try {
      await onSendRequest({ username: user.username || user.display_name, targetUser: user });
      setStatus('success');
      setTimeout(() => {
        setStatus(null);
        setSearch('');
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Add Friend" size="sm">
      <div className="space-y-4">
        <p className="text-sm text-zinc-500">Search by username or display name</p>
        ...
        <Input
          ...
          rightIcon={isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
          autoFocus
        />
        ...
        {search.length >= 2 && searchResults.length === 0 && !isLoading && (
