      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button onClick={onClick}
            ...
            style={{ color: active ? '#f87171' : '#666', background: 'transparent' }}>
            {children}
          </button>
        </TooltipTrigger>
        <TooltipContent side="top" className="bg-black text-white text-xs border-0 px-2 py-1">
          {label}
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
...
export default function UserPanel({ profile, isMuted, isDeafened, onToggleMute, onToggleDeafen, onSettings }) {
  const name = profile?.display_name || 'User';
  const status = profile?.status || 'online';
  
  return (
    <div className="h-[52px] px-2 flex items-center gap-1" style={{ background: '#111' }}>
      <div className="flex items-center gap-2 flex-1 min-w-0 px-1">
        ...
        <div className="min-w-0">
          ...
          <div className="text-[10px] text-zinc-500 leading-tight capitalize">{status}</div>
        </div>
      </div>
